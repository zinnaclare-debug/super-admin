  ...
  return (
    <div className="announce-page">
      <section className="announce-hero">
        <div>
          <span className="announce-pill">Staff Announcement Desk</span>
          <h2>Daily updates for your teaching workflow</h2>
          <p className="announce-subtitle">
            Track school-wide notices, level updates, and coordination messages in one colorful stream.
          </p>
          <div className="announce-metrics">
            <span>{loading ? "Syncing..." : `${items.length} active update${items.length === 1 ? "" : "s"}`}</span>
            <span>{error ? "Attention needed" : "Live notices enabled"}</span>
          </div>
        </div>
